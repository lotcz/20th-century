...
export default class CockpitController extends ControllerBase {
	...
	onClick(coords) {
		...
		const target = new Vector2(
			this.model.cockpit.background.coordinates.x + (coords.x - center.x),
			this.model.cockpit.background.coordinates.y + (coords.y - center.y)
		);
		this.addChild(
	...
	onMouseMove(coords) {
		...
		const imageCenter = this.model.cockpit.background.size.multiply(0.5);
		const target = new Vector2(
			imageCenter.x + (diff.x * imageCenter.x),
			imageCenter.y + (diff.y * imageCenter.y)
		);

		this.model.cockpit.custom.set(target.toString());
