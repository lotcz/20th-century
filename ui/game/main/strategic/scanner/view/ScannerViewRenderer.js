...
export default class ScannerViewRenderer extends DomRenderer {
	...
	constructor(game, model, dom) {
		...
		this.addAutoEvent(
			this.model.main.strategic.scannerPanel.scannerViewSize,
			'change',
			() => this.resize(),
			true
		);
	...
	renderInternal() {
		...
		if (this.model.main.globe.ufo.ufoPosition.isDirty) {
			this.updateCameraPosition();
		}
	...
	updateCameraPosition() {
		const position= this.model.main.globe.ufo.ufoPosition;
	...
	resize() {
		const size = this.model.main.strategic.scannerPanel.scannerViewSize;
	...
	restoreSceneFromCache() {
		if (this.model.main.cachedScannerScene) {
			this.renderer = this.model.main.cachedScannerScene.renderer;
			this.camera = this.model.main.cachedScannerScene.camera;
			this.scene = this.model.main.cachedScannerScene.scene;
			this.earthGroup = this.model.main.cachedScannerScene.earthGroup;
			return;
		}
		...
		this.camera = new THREE.PerspectiveCamera(
			33,
			this.model.main.strategic.scannerPanel.scannerViewSize.x / this.model.main.strategic.scannerPanel.scannerViewSize.y,
			0.1,
		...
		this.model.main.cachedScannerScene = {
	...
	destroyCache() {
		...
		this.scene = null;
		this.camera = null;
		this.model.main.cachedScannerScene = null;
	}
}
